...
void MCP23008::setDefaultValue(uint8_t reg) {
  write8(MCP23008_DEFVAL, reg);
}
void MCP23008::setDefaultValuePin(uint8_t p, uint8_t v) {
...
uint8_t MCP23008::defaultValue() {
  return read8(MCP23008_DEFVAL);
}

uint8_t MCP23008::defaultValuePin(uint8_t p) {
  uint8_t reg = defaultValue();
  return (reg & (1 << p));
}


void MCP23008::setInterruptOnChange(uint8_t reg) {
  write8(MCP23008_GPINTEN, reg);
}

void MCP23008::setInterruptOnChangePin(uint8_t p, bool enabled, CompareTo compareto) {
  if (p > 7) {
    return;
  }
...
uint8_t MCP23008::interruptOnChange() {
  return read8(MCP23008_GPINTEN);
}

bool MCP23008::interruptOnChangePin(uint8_t p) {
  if (p > 7) {
    return false;
  }
...
void MCP23008::setInterruptCompare(uint8_t reg) {
  write8(MCP23008_INTCON, reg);
}



void MCP23008::setInterruptComparePin(uint8_t p, CompareTo compareto) {
...
uint8_t MCP23008::interruptFlags() {
  return read8(MCP23008_INTF);
}

uint8_t MCP23008::interruptCapture() {
  return read8(MCP23008_INTCAP);
}

bool MCP23008::interruptOnPin(uint8_t p) {
  uint8_t reg = interruptFlags();
  return (reg & (1 << p));
}


void MCP23008::setConfiguration(const IOConfiguration& config) {
  write8(MCP23008_IOCON, config.registerValue);
}

MCP23008::IOConfiguration MCP23008::configuration() {
  MCP23008::IOConfiguration conf;
  conf.registerValue = read8(MCP23008_IOCON);
  return conf;
}

void MCP23008::setInterruptPolarity(MCP23008::InterruptPolarity pol) {
  MCP23008::IOConfiguration conf = configuration();
  conf.intpol = pol;
  setConfiguration(conf);
}

MCP23008::InterruptPolarity MCP23008::interruptPolarity() {
  MCP23008::IOConfiguration conf = configuration();
  return conf.intpol;
}

void MCP23008::setInterruptOutput(MCP23008::InterruptOutput intout) {
  MCP23008::IOConfiguration conf = configuration();
  conf.odr = intout;
  setConfiguration(conf);
}

MCP23008::InterruptOutput MCP23008::interruptOutput() {
  MCP23008::IOConfiguration conf = configuration();
  return conf.odr;
}
